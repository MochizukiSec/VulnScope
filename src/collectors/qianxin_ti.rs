...
pub struct QianxinTiCollector {
    client: Client,
    base_url: String,
}

impl QianxinTiCollector {
    pub fn new() -> Self {
        Self {
            client: Client::builder()
                ...
                .build()
                .unwrap(),
            base_url: "https://ti.qianxin.com".to_string(),
        }
    }
    ...
    async fn fetch_vulnerability_list(&self) -> Result<Vec<VulnItem>> {
        ...
        // 奇安信威胁情报中心通常提供RSS订阅或者漏洞列表页面
        // 这里我们采用爬取最新漏洞列表的方式
        let urls = vec![
            "https://ti.qianxin.com/vuln",
            "https://ti.qianxin.com/advisory",
        ];
        ...
        for url in urls {
            match self.fetch_page_vulnerabilities(url).await {
                ...
                Err(e) => {
                    tracing::warn!("获取奇安信威胁情报页面失败 {}: {}", url, e);
                    continue;
                }
            }
            
            // 添加延迟避免被反爬虫
            sleep(Duration::from_millis(2000 + fastrand::u64(0..1000))).await;
        }
    ...
    async fn fetch_page_vulnerabilities(&self, url: &str) -> Result<Vec<VulnItem>> {
        let response = self.client
            .get(url)
            .header("Referer", "https://ti.qianxin.com/")
    ...
    fn parse_vulnerability_list(&self, html: &str) -> Result<Vec<VulnItem>> {
        ...
        for selector_str in selectors {
            if let Ok(selector) = Selector::parse(selector_str) {
                ...
                if !items.is_empty() {
                    tracing::info!("使用选择器 {} 找到 {} 个漏洞项目", selector_str, items.len());
                    
                    for item in items.into_iter().take(10) { // 限制数量
    ...
    fn parse_vulnerability_item(&self, html: &str) -> Option<VulnItem> {
        ...
        let title = self.extract_text_by_selectors(&fragment, &[
            "h3", "h4", ".title", ".name", ".vuln-title", "a"
        ])?;

        // 提取描述
        let description = self.extract_text_by_selectors(&fragment, &[
            ".description", ".desc", ".content", ".summary", "p"
    ...
    fn parse_severity(&self, level: &str) -> Severity {
        match level {
            "严重" | "极高" | "critical" => Severity::Critical,
            "高危" | "高" | "high" => Severity::High,
            "中危" | "中" | "medium" => Severity::Medium,
            "低危" | "低" | "low" => Severity::Low,
            _ => Severity::Unknown,
        }
    }
    ...
    fn get_sample_vulnerabilities(&self) -> Vec<VulnItem> {
        vec![
            VulnItem {
                title: "Apache Struts2远程代码执行漏洞威胁情报".to_string(),
                description: "Apache Struts2框架存在远程代码执行漏洞，攻击者可通过构造恶意请求执行任意代码".to_string(),
                cve_id: Some("CVE-2023-50164".to_string()),
                severity: "严重".to_string(),
                cvss_score: Some(9.8),
                published_date: Utc::now(),
                source_url: Some("https://ti.qianxin.com/vuln/struts2-rce".to_string()),
                tags: vec!["RCE".to_string(), "Apache".to_string(), "Web应用".to_string()],
            },
            VulnItem {
                title: "Windows内核权限提升漏洞威胁分析".to_string(),
                description: "Windows操作系统内核存在权限提升漏洞，本地攻击者可利用该漏洞获取系统权限".to_string(),
                cve_id: Some("CVE-2023-36033".to_string()),
                severity: "高危".to_string(),
                cvss_score: Some(7.8),
                published_date: Utc::now() - chrono::Duration::hours(12),
                source_url: Some("https://ti.qianxin.com/vuln/windows-lpe".to_string()),
                tags: vec!["权限提升".to_string(), "Windows".to_string(), "内核".to_string()],
            },
        ]
    }
...
impl VulnerabilityCollector for QianxinTiCollector {
    fn name(&self) -> &str {
        "奇安信威胁情报中心"
    }

    fn description(&self) -> &str {
        "奇安信威胁情报中心，提供专业的漏洞威胁情报分析"
    }

    async fn collect(&self) -> Result<Vec<Vulnerability>> {
        tracing::info!("开始从奇安信威胁情报中心收集漏洞数据");
        ...
        tracing::info!("从奇安信威胁情报中心收集到 {} 个漏洞", all_vulnerabilities.len());
